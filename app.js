...
function buildFeasDiagram() {
  ...
  container.innerHTML = `
    <div style="position: relative; width: 100%; max-width: 500px; aspect-ratio: 1; background: #f5f5f5; border: 2px solid #ddd; border-radius: 8px; margin: 0 auto;">
      <div id="lotLabel" style="position: absolute; top: 10px; left: 10px; font-size: 14px; font-weight: 600; background: white; padding: 4px 8px; border-radius: 4px;">Lot: —</div>
      <div id="buildableRect" style="position: absolute; top: 10%; left: 10%; width: 80%; height: 70%; border: 2px dashed #4D5382; background: rgba(77,83,130,0.05);"></div>
      <div id="primaryRect" style="position: absolute; bottom: 15%; left: 10%; width: 40%; height: 35%; background: #084C61; border-radius: 4px; display: flex; align-items: center; justify-content: center; color: white; font-size: 12px;">Primary</div>
      <div id="aduRect" style="position: absolute; top: 15%; right: 10%; width: 30%; height: 25%; background: #432818; border-radius: 4px; display: flex; align-items: center; justify-content: center; color: white; font-size: 11px;">ADU</div>
    </div>
  `;
}
...
function runFeasibilityCheck() {
  ...
  const lotSize = parseFloat(document.getElementById("feasLotSize").value) || 0;
  ...
  const zoneRows = cityRows.filter(r => safeText(r[zoneIdx]).trim() === zone);

  if (!zoneRows.length) {
    summaryEl.innerHTML = "<p>No data found for this city/zone.</p>";
    return;
  }
  ...
  const lotOK = isNaN(minLot) || lotSize >= minLot;
  ...
  if (!allowed) {
    ...
    status = "yes";
    msg = "Lot and ADU size appear feasible (subject to full review).";
  } else if (!lotOK) {
    status = "maybe";
    msg = "Lot size below minimum. May need variance.";
  } else {
    status = "maybe";
    msg = "ADU size exceeds maximum. Consider smaller ADU.";
  }
  ...
  document.getElementById("lotLabel").textContent = `Lot: ${lotSize ? lotSize.toLocaleString() + " sf" : "—"}`;
}
