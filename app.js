...
async function loadPermitsData() {
  try {
    ...
    // Then drop obviously malformed rows that don't even include the full
    // header width. The permits CSV contains occasional stray lines like
    // "Status: Accepted" that should not be treated as real records.
    const width = permitHeaders.length;
    const shapedRows = nonEmptyRows
      .filter((row) => row.length >= width)
      .map((row) => row.slice(0, width));

    // Keep the cleaned set as-is; the CSV is already scoped to ADUs and
    // filtering heuristics were discarding valid rows. By trusting the CSV
    // and normalizing the width, we ensure the permit table always renders
    // data instead of getting stuck behind an empty-state overlay.
    permitRows = shapedRows;
...
function renderPermits() {
  ...
  if (!permitRows.length) {
    ...
    const tr = document.createElement("tr");
    const td = document.createElement("td");
    td.colSpan = PERMIT_TABLE_COLUMNS;
    td.className = "table-empty-row";
